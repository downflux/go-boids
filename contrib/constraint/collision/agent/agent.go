...
func (c C) Force(a agent.A) vector.V {
	r := vector.Sub(a.P(), c.o.Obstacle.P())
	separation := vector.Magnitude(r) / (c.o.Obstacle.R() + a.R() + a.MaxVelocity().R())

	if epsilon.Absolute(1e-5).Within(vector.Magnitude(r), 0) {
		return *vector.New(0, 0)
	}

	return vector.Scale(
		c.o.K/separation/separation,
		vector.Unit(r),
	)
	...
	/*
		...
		r := a.R() + c.o.Obstacle.R() + vector.Magnitude(v)
		...
		if ok {
			...
			if collision := t > 0; collision {
				...
				// hard as possible.
				scalar := 1.0 / math.Max(1e-5, vector.Magnitude(avoid)-r)

...
var (
	...
	Mass         = 2000.0
	MaxVelocity  = *polar.New(80.0, 0.5*math.Pi)
	Radius       = 4.0
...
func (l Lambo) Agent() *config.A {
	...
	return &config.A{
		O: config.O{
			ID:           agent.ID(fmt.Sprintf("(%.3f, %.3f)", l.P.X(), l.P.Y())),
			P:            l.P,
			V:            velocity,
			R:            Radius * math.Pow(l.MassMultiplier, 1.5),
			Goal:         l.Goal,
			Mass:         Mass * l.MassMultiplier,
			Heading:      heading,
			MaxNetTorque: MaxNetTorque,
			MaxNetForce:  MaxNetForce,
			MaxVelocity: *polar.New(
				MaxVelocity.R()/math.Pow(l.MassMultiplier, 1.1),
				MaxVelocity.Theta()/math.Pow(l.MassMultiplier, 2),
			),
		},
	}
}
...
func GenerateGrid(h int, w int) config.C {
	...
	for i, p := range positions {
		c.Agents = append(c.Agents, Lambo{
			P:              p,
			Goal:           goals[i],
